...
# ref_img=ref_img-ref_img.min()/(ref_img.max()-ref_img.min()) #normalize to [0-1]
...
def calculate_objectives(fp):
    ...
    if len(shp)>3:
        ...
        target=ref_img
    # rec_loss=(fp-target)**2
    # rec_loss=rec_loss.view(shp[0],-1).mean(1,keepdim=True)
    rec_loss=torch.nn.functional.mse_loss(fp,target)
    err=rec_loss+kl_div
    return err.item()

def FWA_orig(model_G,*args):
    
    n_fireworks = 5         # how many fireworks in the swarm, aka how many solutions
    ...
    for i in range(n_fireworks):
        ...
        objective_values[i]=calculate_objectives(img)
            ...
            for j in range(num_explosion_spark[i]):
                if fitcount<FES:
                    ...
                    with torch.no_grad():
                        ...
                        img=model_G(ltnt,*args).squeeze(0).clamp(min=-1, max=1)
                    obj_val_explosion_spark[index]=calculate_objectives(img)
